...
def test_attributes():
    ...
    assert classifier.algorithm == ClassificationAlgorithmEnum.CATBOOST
    assert isinstance(classifier.base_estimator, ctb.CatBoostClassifier)
    assert classifier.param_grid == clfcfg.CATBOOST_PARAM_GRID
...
def test_train_cv(mock_calibration, mock_search_cv):
    ...
    mock_classifier_best_estimator = MagicMock(spec=ctb.CatBoostClassifier)
    mock_search_cv.best_estimator_.return_value = mock_classifier_best_estimator

    mock_classifier_calibrated_classifier = MagicMock(spec=ctb.CatBoostClassifier)
...
@mock.patch('catboost.CatBoostClassifier.fit')
@mock.patch('src.classification.algorithm.classifier.CalibratedClassifierCV')
def test_train_no_cv(mock_calibration, mock_classifier_fit):
    ...
    CatBoostClassifier().train(valid_x, valid_y)
...
@mock.patch('catboost.CatBoostClassifier.predict_proba')
@mock.patch('catboost.CatBoostClassifier.predict')
def test_predict(mock_classifier_predict, mock_classifier_predict_proba):
    ...
    classifier = CatBoostClassifier()
    classifier.classifier = ctb.CatBoostClassifier()
...
def test_predict_none_classifier():
    ...
    with pytest.raises(Exception) as e_info:
        CatBoostClassifier().predict(valid_x)
...
def test_evaluate_none_classifier():
    ...
    valid_y = pd.DataFrame({'col': ['1']})
    with pytest.raises(Exception) as e_info:
        CatBoostClassifier().evaluate(valid_x, valid_y)
