...
def test_attributes():
    ...
    assert classifier.algorithm == ClassificationAlgorithmEnum.KNN
    assert isinstance(classifier.base_estimator, KNeighborsClassifier)
    assert classifier.param_grid == clfcfg.KNN_PARAM_GRID
...
def test_train_cv(mock_calibration, mock_search_cv):
    ...
    mock_classifier_best_estimator = MagicMock(spec=KNeighborsClassifier)
    mock_search_cv.best_estimator_.return_value = mock_classifier_best_estimator

    mock_classifier_calibrated_classifier = MagicMock(spec=KNeighborsClassifier)
...
@mock.patch('sklearn.neighbors.KNeighborsClassifier.fit')
@mock.patch('src.classification.algorithm.classifier.CalibratedClassifierCV')
def test_train_no_cv(mock_calibration, mock_classifier_fit):
    ...
    KNNClassifier().train(valid_x, valid_y)
...
@mock.patch('sklearn.neighbors.KNeighborsClassifier.predict_proba')
@mock.patch('sklearn.neighbors.KNeighborsClassifier.predict')
...
def test_evaluate_none_classifier():
    ...
    valid_y = pd.DataFrame({'col': ['1']})
    with pytest.raises(Exception) as e_info:
        KNNClassifier().evaluate(valid_x, valid_y)
