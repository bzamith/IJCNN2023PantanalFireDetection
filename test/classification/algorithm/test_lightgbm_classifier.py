...
def test_attributes():
    ...
    assert classifier.algorithm == ClassificationAlgorithmEnum.LIGHTGBM
    assert isinstance(classifier.base_estimator, LGBMClassifier)
    assert classifier.param_grid == clfcfg.LGBM_PARAM_GRID
...
def test_train_cv(mock_calibration, mock_search_cv):
    ...
    mock_classifier_best_estimator = MagicMock(spec=LGBMClassifier)
    mock_search_cv.best_estimator_.return_value = mock_classifier_best_estimator

    mock_classifier_calibrated_classifier = MagicMock(spec=LGBMClassifier)
...
@mock.patch('lightgbm.LGBMClassifier.fit')
@mock.patch('src.classification.algorithm.classifier.CalibratedClassifierCV')
def test_train_no_cv(mock_calibration, mock_classifier_fit):
    ...
    LightGBMClassifier().train(valid_x, valid_y)
...
@mock.patch('lightgbm.LGBMClassifier.predict_proba')
@mock.patch('lightgbm.LGBMClassifier.predict')
def test_predict(mock_classifier_predict, mock_classifier_predict_proba):
    ...
    classifier = LightGBMClassifier()
    classifier.classifier = LGBMClassifier()
...
def test_predict_none_classifier():
    ...
    with pytest.raises(Exception) as e_info:
        LightGBMClassifier().predict(valid_x)
...
def test_evaluate_none_classifier():
    ...
    valid_y = pd.DataFrame({'col': ['1']})
    with pytest.raises(Exception) as e_info:
        LightGBMClassifier().evaluate(valid_x, valid_y)
