...
def test_attributes():
    ...
    assert classifier.algorithm == ClassificationAlgorithmEnum.NB
    assert isinstance(classifier.base_estimator, GaussianNB)
    assert classifier.param_grid == clfcfg.NAIVE_BAYES_PARAM_GRID
...
def test_train_cv(mock_calibration, mock_search_cv):
    ...
    mock_classifier_best_estimator = MagicMock(spec=GaussianNB)
    mock_search_cv.best_estimator_.return_value = mock_classifier_best_estimator

    mock_classifier_calibrated_classifier = MagicMock(spec=GaussianNB)
...
@mock.patch('sklearn.naive_bayes.GaussianNB.fit')
@mock.patch('src.classification.algorithm.classifier.CalibratedClassifierCV')
def test_train_no_cv(mock_calibration, mock_classifier_fit):
    ...
    NaiveBayesClassifier().train(valid_x, valid_y)
...
@mock.patch('sklearn.naive_bayes.GaussianNB.predict_proba')
@mock.patch('sklearn.naive_bayes.GaussianNB.predict')
...
def test_predict_none_classifier():
    ...
    with pytest.raises(Exception) as e_info:
        NaiveBayesClassifier().predict(valid_x)
...
def test_evaluate_none_classifier():
    ...
    valid_y = pd.DataFrame({'col': ['1']})
    with pytest.raises(Exception) as e_info:
        NaiveBayesClassifier().evaluate(valid_x, valid_y)
