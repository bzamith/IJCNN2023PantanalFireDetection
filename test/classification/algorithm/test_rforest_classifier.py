...
def test_attributes():
    ...
    assert classifier.algorithm == ClassificationAlgorithmEnum.RFOREST
    assert isinstance(classifier.base_estimator, ensemble.RandomForestClassifier)
    assert classifier.param_grid == clfcfg.RANDOM_FOREST_PARAM_GRID
...
def test_train_cv(mock_calibration, mock_search_cv):
    ...
    mock_classifier_best_estimator = MagicMock(spec=ensemble.RandomForestClassifier)
    mock_search_cv.best_estimator_.return_value = mock_classifier_best_estimator

    mock_classifier_calibrated_classifier = MagicMock(spec=ensemble.RandomForestClassifier)
...
@mock.patch('sklearn.ensemble.RandomForestClassifier.fit')
@mock.patch('src.classification.algorithm.classifier.CalibratedClassifierCV')
def test_train_no_cv(mock_calibration, mock_classifier_fit):
    ...
    RandomForestClassifier().train(valid_x, valid_y)
...
@mock.patch('sklearn.ensemble.RandomForestClassifier.predict_proba')
@mock.patch('sklearn.ensemble.RandomForestClassifier.predict')
def test_predict(mock_classifier_predict, mock_classifier_predict_proba):
    ...
    classifier = RandomForestClassifier()
    classifier.classifier = ensemble.RandomForestClassifier()
...
def test_predict_none_classifier():
    ...
    with pytest.raises(Exception) as e_info:
        RandomForestClassifier().predict(valid_x)
...
def test_evaluate_none_classifier():
    ...
    valid_y = pd.DataFrame({'col': ['1']})
    with pytest.raises(Exception) as e_info:
        RandomForestClassifier().evaluate(valid_x, valid_y)
