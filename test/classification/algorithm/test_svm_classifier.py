...
def test_attributes():
    ...
    assert classifier.algorithm == ClassificationAlgorithmEnum.SVM
    assert isinstance(classifier.base_estimator, SVC)
    assert classifier.param_grid == clfcfg.SVM_PARAM_GRID
...
def test_train_cv(mock_calibration, mock_search_cv):
    ...
    mock_classifier_best_estimator = MagicMock(spec=SVC)
    mock_search_cv.best_estimator_.return_value = mock_classifier_best_estimator

    mock_classifier_calibrated_classifier = MagicMock(spec=SVC)
...
@mock.patch('sklearn.svm.SVC.fit')
@mock.patch('src.classification.algorithm.classifier.CalibratedClassifierCV')
def test_train_no_cv(mock_calibration, mock_classifier_fit):
    ...
    SVMClassifier().train(valid_x, valid_y)
...
@mock.patch('sklearn.svm.SVC.predict_proba')
@mock.patch('sklearn.svm.SVC.predict')
...
def test_evaluate_none_classifier():
    ...
    valid_y = pd.DataFrame({'col': ['1']})
    with pytest.raises(Exception) as e_info:
        SVMClassifier().evaluate(valid_x, valid_y)
