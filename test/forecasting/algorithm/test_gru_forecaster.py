...
def test_attributes():
    ...
    assert forecaster.algorithm == ForecastingAlgorithmEnum.GRU
...
def test_build_architecture():
    ...
    actual_X, actual_y, actual_architecture = GRUForecaster().build_architecture(dataset)
...
def test_evaluate_none_forecaster():
    ...
    with pytest.raises(Exception) as e_info:
        GRUForecaster().evaluate(valid_dataset)
...
def test_forecast_none_forecaster():
    valid_dataset = pd.DataFrame({'col': ['value']})
    with pytest.raises(Exception) as e_info:
        GRUForecaster().forecast(valid_dataset)
