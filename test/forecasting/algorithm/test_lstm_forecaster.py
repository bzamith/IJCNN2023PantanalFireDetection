...
def test_attributes():
    ...
    assert forecaster.algorithm == ForecastingAlgorithmEnum.LSTM
...
def test_build_architecture():
    ...
    actual_X, actual_y, actual_architecture = LSTMForecaster().build_architecture(dataset)
...
def test_evaluate_none_forecaster():
    ...
    with pytest.raises(Exception) as e_info:
        LSTMForecaster().evaluate(valid_dataset)
...
def test_forecast_none_forecaster():
    valid_dataset = pd.DataFrame({'col': ['value']})
    with pytest.raises(Exception) as e_info:
        LSTMForecaster().forecast(valid_dataset)
