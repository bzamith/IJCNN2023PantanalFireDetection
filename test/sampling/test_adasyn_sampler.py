...
def test_attributes():
    sampler = ADASYNSampler()
    assert sampler.method == SamplingMethodEnum.ADASYN
    assert isinstance(sampler.base_sampler, ADASYN)
    assert sampler.sampler is None


@mock.patch('imblearn.over_sampling.ADASYN.fit_resample')
def test_fit_sample(mock_sampling_fit):
    ...
    output_x, output_y = ADASYNSampler().fit_sample(valid_x, valid_y, columns=columns)
...
def test_fit_sample_default_columns():
    ...
    with pytest.raises(KeyError) as e_info:
        ADASYNSampler().fit_sample(valid_x, valid_y)
