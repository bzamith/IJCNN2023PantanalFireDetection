...
def test_attributes():
    ...
    assert sampler.method == SamplingMethodEnum.CLUSTER_CENTROIDS
    assert isinstance(sampler.base_sampler, ClusterCentroids)
    assert sampler.sampler is None


@mock.patch('imblearn.under_sampling.ClusterCentroids.fit_resample')
def test_fit_sample(mock_sampling_fit):
    ...
    output_x, output_y = ClusterCentroidsSampler().fit_sample(valid_x, valid_y, columns=columns)
...
def test_fit_sample_default_columns():
    ...
    with pytest.raises(KeyError) as e_info:
        ClusterCentroidsSampler().fit_sample(valid_x, valid_y)
