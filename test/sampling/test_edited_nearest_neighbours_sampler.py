...
def test_attributes():
    sampler = EditedNearestNeighboursSampler()
    assert sampler.method == SamplingMethodEnum.EDITED_NEAREST_NEIGHBOURS
    assert isinstance(sampler.base_sampler, EditedNearestNeighbours)
    assert sampler.sampler is None


@mock.patch('imblearn.under_sampling.EditedNearestNeighbours.fit_resample')
def test_fit_sample(mock_sampling_fit):
    ...
    output_x, output_y = EditedNearestNeighboursSampler().fit_sample(valid_x, valid_y, columns=columns)
...
def test_fit_sample_default_columns():
    ...
    with pytest.raises(KeyError) as e_info:
        EditedNearestNeighboursSampler().fit_sample(valid_x, valid_y)
