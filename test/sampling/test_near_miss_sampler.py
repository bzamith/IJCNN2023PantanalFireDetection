...
def test_attributes():
    ...
    assert sampler.method == SamplingMethodEnum.NEAR_MISS
    assert isinstance(sampler.base_sampler, NearMiss)
    assert sampler.sampler is None


@mock.patch('imblearn.under_sampling.NearMiss.fit_resample')
def test_fit_sample(mock_sampling_fit):
    ...
    output_x, output_y = NearMissSampler().fit_sample(valid_x, valid_y, columns=columns)
...
def test_fit_sample_default_columns():
    ...
    with pytest.raises(KeyError) as e_info:
        NearMissSampler().fit_sample(valid_x, valid_y)
