...
def test_attributes():
    sampler = OneSidedSelectionSampler()
    assert sampler.method == SamplingMethodEnum.ONE_SIDED_SELECTION
    assert isinstance(sampler.base_sampler, OneSidedSelection)
    assert sampler.sampler is None


@mock.patch('imblearn.under_sampling.OneSidedSelection.fit_resample')
def test_fit_sample(mock_sampling_fit):
    ...
    output_x, output_y = OneSidedSelectionSampler().fit_sample(valid_x, valid_y, columns=columns)
...
def test_fit_sample_default_columns():
    ...
    with pytest.raises(KeyError) as e_info:
        OneSidedSelectionSampler().fit_sample(valid_x, valid_y)
