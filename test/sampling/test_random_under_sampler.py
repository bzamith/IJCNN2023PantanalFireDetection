...
def test_attributes():
    ...
    assert sampler.method == SamplingMethodEnum.RANDOM_UNDER_SAMPLER
    assert isinstance(sampler.base_sampler, us.RandomUnderSampler)
    assert sampler.base_sampler.sampling_strategy == 'majority'
    assert sampler.sampler is None


@mock.patch('imblearn.under_sampling.RandomUnderSampler.fit_resample')
def test_fit_sample(mock_sampling_fit):
    ...
    output_x, output_y = RandomUnderSampler().fit_sample(valid_x, valid_y, columns=columns)
...
def test_fit_sample_default_columns():
    ...
    with pytest.raises(KeyError) as e_info:
        RandomUnderSampler().fit_sample(valid_x, valid_y)
