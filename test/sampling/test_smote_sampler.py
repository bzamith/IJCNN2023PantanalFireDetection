...
def test_attributes():
    ...
    assert sampler.method == SamplingMethodEnum.SMOTE
    assert isinstance(sampler.base_sampler, SMOTE)
    assert sampler.sampler is None


@mock.patch('imblearn.over_sampling.SMOTE.fit_resample')
def test_fit_sample(mock_sampling_fit):
    ...
    output_x, output_y = SMOTESampler().fit_sample(valid_x, valid_y, columns=columns)
...
def test_fit_sample_default_columns():
    ...
    with pytest.raises(KeyError) as e_info:
        SMOTESampler().fit_sample(valid_x, valid_y)
