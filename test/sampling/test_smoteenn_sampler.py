...
def test_attributes():
    sampler = SMOTEENNSampler()
    assert sampler.method == SamplingMethodEnum.SMOTEENN
    assert isinstance(sampler.base_sampler, SMOTEENN)
    assert sampler.sampler is None


@mock.patch('imblearn.combine.SMOTEENN.fit_resample')
def test_fit_sample(mock_sampling_fit):
    ...
    output_x, output_y = SMOTEENNSampler().fit_sample(valid_x, valid_y, columns=columns)
...
def test_fit_sample_default_columns():
    ...
    with pytest.raises(KeyError) as e_info:
        SMOTEENNSampler().fit_sample(valid_x, valid_y)
